...
echo ""
echo "  3. Set DNS records BEFORE deploying (Caddy needs these for ACME):"
echo "       A record:  mail.chemtrail-tracker.com  →  <this server's public IP>"
echo "       A record:  chemtrail-tracker.com        →  <this server's public IP>"
echo "     (MX, SPF, DKIM, DMARC records are added after step 5 — see DNS_RECORDS.md)"
echo ""
...
echo "       SMTP_PASSWORD        — password you will set for accounts@chemtrail-tracker.com"
echo ""
echo "  5. Run the first deploy:"
echo "       chmod +x scripts/deploy.sh"
echo "       ./scripts/deploy.sh"
echo ""
echo "  6. Create the mail account and generate DKIM keys (one-time, after deploy):"
echo "       ./scripts/setup-mailserver.sh"
echo "     Then add the remaining DNS records printed by that script (see DNS_RECORDS.md)."
echo ""
